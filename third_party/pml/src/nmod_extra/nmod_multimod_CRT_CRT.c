...
{
    ulong i, j;
    fmpz_t sum, term, product;

    fmpz_init(sum);
    fmpz_init(term);
    fmpz_init(product);
    fmpz_set_ui_array(product, C->data + C->num_primes * C->num_primes, C->num_primes);
    ...
    for (i = 0; i < nb; i++)
    {
        ...
        {
            ulong scaled = nmod_mul(C->inverse_cofactors[j], residues[j][i], C->mod_primes[j]);
            fmpz_set_ui_array(term, C->data + j * C->num_primes, C->num_primes);
            fmpz_mul_ui(term, term, scaled);
            fmpz_add(sum, sum, term);
        }

        fmpz_mod(sum, sum, product);
        out[i] = fmpz_fdiv_ui(sum, C->p);
    }
...
{
#if PML_HAVE_AVX2
    if (C->p < (UWORD(1) << 50)) // small modulus: use SIMD floating-point representation 
